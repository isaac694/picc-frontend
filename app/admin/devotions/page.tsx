...
export default function DevotionsAdminPage() {
  ...
  const requestDeleteConfirmation = () => {
    ...
    confirmDeleteToast({
      ...
      description: date ? `Devotion for ${date}` : 'This devotion will be permanently removed.',
      onConfirm: () => handleConfirmDelete(devotionId),
    });
  };
  ...
  const handleConfirmDelete = async (deleteId: string) => {
    ...
    try {
      const response = await apiFetch(`/api/devotions/${deleteId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
